...
*   **AABB Scatter Exclusion Zones**: `LevelManager._spawnScatter()` generates XZ exclusion rectangles from every fixed obstacle footprint (padded by scatter item half-width + 1.5u) and the player safe zone. Scatter items retry position generation up to 12× and apply a fallback nudge — eliminating soda cans and cereal boxes from spawning inside counter slabs.
